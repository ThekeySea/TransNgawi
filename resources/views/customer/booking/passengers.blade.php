...
@section('content')
    <section class="section-spacing bg-surface" x-data="checkoutTimer('{{ $booking->hold_expires_at->toIso8601String() }}')">
        <div class="container-app">
            ...
            <div class="mb-8">
                <h1 class="text-2xl font-bold text-text sm:text-3xl">Data Penumpang</h1>
                <p class="mt-2 text-sm text-text-muted">Lengkapi data penumpang untuk melanjutkan pemesanan.</p>
            </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('booking.passengers.store', $booking) }}" method="POST">
                                ...
                                <div class="space-y-5">
                                    <div>
                                        <label for="passenger_name" class="input-label">Nama Lengkap</label>
                                        <input type="text" name="passenger_name" id="passenger_name" value="{{ old('passenger_name', $booking->passenger_name) }}" class="input" required>
                                    ...
                                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        <div>
                                            <label for="passenger_email" class="input-label">Email</label>
                                            <input type="email" name="passenger_email" id="passenger_email" value="{{ old('passenger_email', $booking->passenger_email) }}" class="input" required>
                                            @error('passenger_email')
                                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        <div>
                                            <label for="passenger_phone" class="input-label">Telepon</label>
                                            <input type="text" name="passenger_phone" id="passenger_phone" value="{{ old('passenger_phone', $booking->passenger_phone) }}" class="input" required>
                                ...
                                <div class="mt-6 flex gap-3">
                                    <a href="{{ route('booking.seats', $booking->trip_id) }}" class="btn-secondary">Kembali</a>
                                    <button type="submit" class="btn-primary">Lanjut ke Review</button>
                                </div>
                            </form>
                        </div>
                    </div>
